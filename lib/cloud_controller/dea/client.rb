    ...
    module Client
      ...
      class << self
        ...
        def configure(config, message_bus, dea_pool, blobstore_url_generator)
          ...
          if config['dea_client_cert']
            ssl = client.ssl_config
            ssl.verify_mode = OpenSSL::SSL::VERIFY_PEER
            ssl.clear_cert_store

            ssl.add_trust_ca(config['dea_client_ca'])
            ssl.set_client_cert_file(config['dea_client_cert'], config['dea_client_key'])
          end
          @http_client = client
        end
        ...
        def dea_send_start(dea, args)
          dea_id = dea.dea_id
          address = dea.address
          logger.debug "sending 'dea.start' for dea_id: #{dea_id} to #{address} with '#{args}'"

          if dea.address
            begin
              @http_client.post("https://#{address}/v1/apps", header: {'Content-Type'=>'application/json'}, body: MultiJson.dump(args))
            rescue => e
              logger.warn "start failed", dea_id: dea_id, address: address, error: e
            end
          else
